...
def run_pipeline(base_path,client_name,voice_key='female_standard',draft_box=None,progress=None,target_language='zh'):
    ...
    content=ContentGenerator(voice_key=voice_key,target_language=target_language).generate(base_path=base_path,client_name=client_name,keywords=fetch.keywords)
    ...
    return {'script_text':content.script_text,'exported_draft_directory':exported,'selected_voice_key':voice_key,'generated_voice_name':content.voice,'target_language':target_language}
class App:
    def __init__(self):
    ...
    def _page2(self):
        f=ttk.Frame(self.box,style='C.TFrame'); ttk.Label(f,text='2. 选择声音',style='H.TLabel').grid(row=0,column=0,sticky='w'); container=ttk.Frame(f,style='C.TFrame'); container.grid(row=1,column=0,sticky='ew',pady=(12,0)); container.columnconfigure(0,weight=1); container.columnconfigure(1,weight=1)
    ...
    def _worker(self):
        try: selected=self.voice.get().strip(); lang=resolve_target_language_by_voice_label(selected); r=run_pipeline(Path(self.material.get()),self.client.get().strip(),VOICES[selected],Path(self.draft.get()),progress=self._report_progress,target_language=lang); self.root.after(0,lambda:self._ok(r))
